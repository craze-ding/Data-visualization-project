...
#返回全国地区城市确诊人数前5名
def get_r1_data():
    sql="SELECT city,confirm FROM (select city,confirm from details where update_time=(select update_time from details order by update_time desc limit 1)and province not in ('湖北','北京','上海','天津','重庆','香港','台湾') and city not in('境外输入','待确认地区')union all select province as city,sum(confirm) as confirm from details where update_time=(select update_time from details order by update_time desc limit 1) and province in ('北京','上海','天津','重庆') group by province) as a order by confirm DESC LIMIT 5"
    res = query(sql)    
    return res
